...
Исходный config_custom при любом исходе возвращается на место; новая стратегия
...
# Успех = хендшейк прошёл: 2xx с осмысленным телом или 3xx (редирект — TLS уже поднялся).
is_success() {
    local code="$1" size="$2"
    case "$code" in
        3[0-9][0-9]) return 0 ;;
        200) [ "$size" -ge "$MIN_SIZE" ] && return 0 || return 1 ;;
    esac
    return 1
}
