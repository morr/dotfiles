...
./tuning.sh       # подобрать рабочую стратегию, если прежняя сломалась
```

`install.sh` и `uninstall.sh` идемпотентны: повторный запуск на уже настроенной
(или уже вычищенной) машине проходит на «ПРОПУСК» и ничего не ломает. Каждый скрипт
пишет полный лог (`~/dpi-bypass-install-*.log`, `~/dpi-bypass-uninstall-*.log`,
`~/dpi-bypass-tuning-*.log`); если что-то пошло не так, этот файл и есть то, что нужно
показать. Пароль администратора спрашивается в начале; система может переспросить его
посреди работы — это нормально.

`tuning.sh` не разворачивает и не сносит стенд — он перебирает стратегии tpws для 443
на уже установленном сервисе и выбирает ту, что берёт заблокированные хосты. Нужен, когда
провайдер поменял DPI и записанная стратегия перестала работать. Подробности — в разделе
«Подбор стратегии».
...
| Сторож стенда (`tpws` и DNS) | `/usr/local/sbin/zapret-watchdog` | LaunchDaemon `/Library/LaunchDaemons/local.zapret-watchdog.plist`, раз в 30 с |
...
**Симптом, ради которого он есть: интернета нет вообще — ни один сайт не открывается,
хотя сеть на месте.** Это не блокировка: отвалился один из двух узлов, без которых
браузер не доходит никуда. `/usr/local/sbin/zapret-watchdog` раз в 30 секунд проверяет
оба и чинит тот, что сломался. У каждого своя проверка, своя починка и свой счётчик
неудач, так что не поднимающийся узел не мешает чинить второй.

### Узел «zapret» — упавший `tpws`

Отличительный признак: любой сайт отдаёт `ERR_CONNECTION_REFUSED`, а через прокси всё
работает.
...
Проверяются три вещи, при любой неудаче делается `zapret restart`:
...
### Узел «dns» — залипший `dnscrypt-proxy`

Отличительный признак: не резолвится ни один домен, при живой сети и рабочем
`dig @8.8.8.8`. Системный DNS смотрит на `127.0.0.1`, так что мёртвый резолвер
оставляет без имён всю машину, а не только браузер.

Так случилось 10.09.2026: демон с аптаймом 27 дней **держал сокет открытым и принимал
соединения, не отвечая на них** — ни по UDP, ни по TCP. Отсюда проверка настоящим
запросом (`dig +time=2 +tries=1 @127.0.0.1`), а не `nc -z`: порт-то был открыт. Пустой
вывод `dig` признаком сбоя тоже не годится — свои `communications error` он пишет
в stdout, — поэтому смотрится код возврата, а следом сам адрес: так ловится и SERVFAIL.

Чинится это `launchctl kickstart -k system/homebrew.mxcl.dnscrypt-proxy`, а если через
5 секунд резолвер не ожил — `pkill -9`. Жёсткий сигнал здесь не перестраховка: в тот раз
демон не отреагировал на SIGTERM от `kickstart`, а поднятый следом инстанс намертво встал
в `bind()` на всё ещё занятом порту (видно в `sample` по `__bind` в libsystem_kernel).
Освободил порт только SIGKILL; заново демон поднялся сам, по `KeepAlive` из своего plist.

В офлайне сторож молчит: без маршрута по умолчанию резолвер не ответит и здоровым.

### Логи

`/var/log/zapret-watchdog.log` пишется только по событиям, в здоровом состоянии пустой;
строки помечены узлом — `[zapret]` или `[dns]`:
...
Если починка не помогла, сторож не долбит узел каждые полминуты: следующая попытка не
раньше чем через 5 минут, счётчик неудач — в `/var/run/zapret-watchdog.<узел>.state`.
Поэтому лог ограничен сверху даже при полностью сломанном стенде.

**По этому логу и надо смотреть, если сбой повторится.** Первопричина ни одного из двух
сбоев так и не известна: краш-репортов macOS по `tpws` не оставил,
`/tmp/darkware-zapret.error.log` пуст, а `dnscrypt-proxy` до 10.09.2026 вообще никуда не
писал — в его plist от Homebrew нет ни `StandardOutPath`, ни `StandardErrorPath`. Теперь
`install.sh` прописывает ему `log_file` (`/opt/homebrew/var/log/dnscrypt-proxy.log`,
ротацию демон делает сам), так что в следующий раз будет что читать. Частота срабатываний
в логе — единственный способ отличить разовый случай от регулярного.
...
## Подбор стратегии (`tuning.sh`)

Стратегия обхода — не навсегда: провайдер меняет DPI, и однажды рабочая строка 443
перестаёт брать сайты. `tuning.sh` перебирает набор стратегий tpws для 443 на уже
установленном сервисе и выбирает ту, что открывает заблокированные хосты, — чтобы не
подбирать вручную под стрессом.

```bash
./tuning.sh                      # без ключей — спросит, что делать (режим, цели, Chrome), и переберёт
./tuning.sh --dry-run            # только измерить и показать, ничего не менять
./tuning.sh --yes                # применить лучшую без вопроса
./tuning.sh --host rutracker.org # мерить на своих хостах (флаг можно повторять)
./tuning.sh --help               # все опции
```

Как и `uninstall.sh`, без единого ключа скрипт сначала спрашивает план: мерить на
стандартных целях или своих и перепроверять ли победителя через Chrome. Само применение
он не навязывает в начале — найдя рабочую стратегию, спрашивает в конце, ставить ли её.
Любой переданный ключ опрос отключает — тогда работает по флагам.

Как он работает и почему так:

- **Кандидаты — сокетные стратегии для 443** (`--split-pos`, `--disorder`, `--oob` в
  разных сочетаниях), список в начале скрипта, правится руками. Пакетных приёмов на
  macOS нет (см. «Потолок метода»), перебирать нечего кроме способа нарезки ClientHello.
- **Каждого кандидата** скрипт подставляет в `config_custom`, перезапускает сервис
  (без пароля — правило sudoers) и замеряет цели через `curl`. Успех — код 2xx с
  осмысленным телом или редирект 3xx: значит TLS-хендшейк прошёл, SNI не срезали.
- **Цель по умолчанию — `www.youtube.com`**: его режут по SNI, но обход чинит (в таблице
  «Что работает» он в рабочих). Больше целей — флагом `--host`, тогда хендшейки размажутся
  по нескольким хостам. `rutracker`/`x.com`/`discord` сюда не годятся — их душат по
  входящему потоку, обходом не лечится, они провалят любую стратегию и испортят замер.
- **Контрольный хост** (`example.com`, не в блок-листе) проверяется первым в каждом
  раунде: если он не отвечает, значит лёг туннель или DNS, и провал целей в этом раунде
  ничего не значит — раунд отбрасывается.
- **Пауза между замерами** (по умолчанию 8 с) и короткий список кандидатов — чтобы не
  словить блокировку хоста за частые хендшейки: после ~15 подряд провайдер режет адрес
  наглухо на час-другой (см. «Тупики»). Скрипт печатает, сколько хендшейков придётся на
  хост, и предупреждает при приближении к порогу.
- **Победителя перепроверяет headless Chrome** (если установлен): у `curl` и Chrome
  разный ClientHello и ломаются они по-разному, поэтому успех по curl подтверждается
  браузером — по размеру DOM, а не по заголовку вкладки (`--no-chrome` отключает).
- **Откат по умолчанию.** Как бы скрипт ни завершился — по Ctrl-C, по ошибке или без
  применения, — исходный `config_custom` возвращается на место и сервис перезапускается
  с ним. Новую стратегию скрипт оставляет, только если ты согласился её применить
  (интерактивный вопрос или `--yes`).

Если **ни одна** стратегия не взяла ни одной цели — скрипт ничего не меняет и говорит об
этом: скорее всего цели сейчас режутся по IP или входящему потоку (обходом не лечится),
либо провайдер уже прижал хосты за перебор — тогда подожди час-другой и повтори.

Нашёл рабочую и хочешь сделать её эталоном на будущее — впиши ту же строку 443 в
`config_custom.working`, тогда `install.sh` будет ставить уже её.
